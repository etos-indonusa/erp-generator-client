...
import { EngineerRtiIdentifyReportService, EngineerRtiIdentifyService } from 'src/sdk/core/services';
import { AmimsEngineerRtiIdentifyDto } from 'src/sdk/core/models';
...
@Component({
    selector: 'app-engineer-rti-identify-share-list',
    templateUrl: './engineer-rti-identify-share-list.component.html',
    styleUrl: './engineer-rti-identify-share-list.component.scss'
})
export class EngineerRtiIdentifyShareListComponent {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        this.filter.status_engineerRtiIdentify = this.status == 'semua' ? null : this.status;
  ...
  warantyStartHoursMax: null
    ...
    searchData(reset: boolean = false): void {
        ...
        finalFilter[`id_engineer_rti_identify`] = { isNotNull: 'aktif' };

        this.engineerRtiIdentifyReportService.engineerRtiIdentifyReportControllerFindAll({
            body: {
                filter: finalFilter,
                joinWhere: [
                                        {
  ...
  },
  {
    ...
    onColumnToggle(): void {
        localStorage.setItem('engineerRtiIdentify_columns', JSON.stringify(this.columns));
    }
    ...
    loadColumnSettings(): void {
        const saved = localStorage.getItem('engineerRtiIdentify_columns');
        if (saved) {
            try {
            ...
            if (Array.isArray(parsed)) {
                ...
                console.warn('Gagal parse engineerRtiIdentify dari localStorage', e);
            }
        }
        }
    // TABLE DINAMIS 
    ...
    add() {
    if (!this.acl.can('engineer-rti-identify', 'can_add') || !this.enable_crud) return;

        const drawerRef = this.drawerService.create<EngineerRtiIdentifyShareAddComponent, {}, string>({
            nzTitle: 'Add',
            nzContent: EngineerRtiIdentifyShareAddComponent,
        nzWidth: (500) + 'px',
        });
    ...
    detail(data:AmimsEngineerRtiIdentifyDto) {
        if (!this.acl.can('contract-site', 'can_list')) return;

        const drawerRef = this.drawerService.create<EngineerRtiIdentifyShareDetailComponent, {}, string>({
            nzTitle: 'Detail',
            nzContent: EngineerRtiIdentifyShareDetailComponent,
            nzWidth: (window.innerWidth * 0.8) + 'px',
            nzContentParams:{
                idEngineerRtiIdentify:data.idEngineerRtiIdentify
            }
        });
