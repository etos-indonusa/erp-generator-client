...
import { PurGrnReportService, PurGrnService } from 'de-sdk-core/lib/services';
import { AmimsPurGrnDto } from 'de-sdk-core/lib/models';
...
@Component({
    selector: 'app-pur-grn-share-list',
    templateUrl: './pur-grn-share-list.component.html',
    styleUrl: './pur-grn-share-list.component.scss'
})
export class PurGrnShareListComponent {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        this.filter.status_purGrn = this.status == 'semua' ? null : this.status;
  ...
  validasiMax: null
    ...
    search_field: string[] = ["noteGrn","numberGrn"];
  ...
  validasiMax: null
    ...
    searchData(reset: boolean = false): void {
        ...
        finalFilter[`id_pur_grn`] = { isNotNull: 'aktif' };

        this.purGrnReportService.purGrnReportControllerFindAll({
            body: {
                filter: finalFilter,
                joinWhere: [
                                        {
  ...
  },
  {
    ...
    columns = [ 
         { key: 'createdAt',  show: true },
              ...
              { key: 'dateQaClose',  show: true },
              { key: 'grnKe',  show: true },
              { key: 'isDeleted',  show: true },
              { key: 'noIndex',  show: true },
              { key: 'noteGrn',  show: true },
    ...
    onColumnToggle(): void {
        localStorage.setItem('purGrn_columns', JSON.stringify(this.columns));
    }
    ...
    loadColumnSettings(): void {
        const saved = localStorage.getItem('purGrn_columns');
        if (saved) {
            try {
            ...
            if (Array.isArray(parsed)) {
                ...
                console.warn('Gagal parse purGrn dari localStorage', e);
            }
        }
        }
    // TABLE DINAMIS 
    ...
    add() {
    if (!this.acl.can('pur-grn', 'can_add') || !this.enable_crud) return;

        const drawerRef = this.drawerService.create<PurGrnShareAddComponent, {}, string>({
            nzTitle: 'Add',
            nzContent: PurGrnShareAddComponent,
        nzWidth: (500) + 'px',
        });
    ...
    detail(data:AmimsPurGrnDto) {
        if (!this.acl.can('contract-site', 'can_list')) return;

        const drawerRef = this.drawerService.create<PurGrnShareDetailComponent, {}, string>({
            nzTitle: 'Detail',
            nzContent: PurGrnShareDetailComponent,
            nzWidth: (window.innerWidth * 0.8) + 'px',
            nzContentParams:{
                idPurGrn:data.idPurGrn
            }
        });
