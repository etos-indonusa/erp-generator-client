...
@Component({
    selector: 'app-schedule-maintenance-oh',
    templateUrl: './schedule-maintenance-oh.component.html',
    styleUrl: './schedule-maintenance-oh.component.scss'
})
export class ScheduleMaintenanceOhComponent {
    @Input('data') data: any[] = [];
    ...
    simulateTotalDue(masterData: any[]): (any & { totalDue: number })[] {
        ...
        for (const item of masterData) {
            ...
            if (type_due === 'DY' && interval > 0) {
                ...
                const future = new Date(installDate);
                ...
                if (future >= dueDate) {
                    ...
                    for (let i = 0; i < totalDue; i++) {
                        const currentPartEach = new Date(installDate);
                        currentPartEach.setDate(currentPartEach.getDate() + daysInterval * i);

                        const dueDateEach = new Date(installDate);
