...
import { PartInstallReportService, PartInstallService, PartService } from 'de-sdk-core';
@Component({
...
export class AircraftShareHeaderComponent {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes['data'] ) {
            ...
            {
                this.data.partAirframe = [];
                this.data.partEngine = [];
                this.getPartAirframe();
                this.getPartEngine();
            }
        }
    }
    ...
    getPartAirframe() {
        this.partService.partControllerFindAll({
            filter: JSON.stringify({
                sn: this.data.serialNumber,
            }),
        }).subscribe(
            data => {
                this.data.partAirframe = data.data
            }
        );
    }
    ...
    getPartEngine() {
        this.partInstallService.partInstallReportControllerFindAll({
            body: {
                filter: {
                    idAircraft: this.data.idAircraft,
                    parent: "0"
                }, 
                include: [
                    {
            ...
            data => {
                this.data.partEngine = data.data?.filter(x => x.part?.sn != this.data.serialNumber);
            }
        );
    }
